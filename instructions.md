# Projeto MIRASWEAR — Instruções e Preferências
...
5. UI/UX e comportamento esperado
- Header/search
  - Desktop: busca inline que expande com animação ao clicar no ícone de lupa; ESC ou clique fora fecha; ao pressionar Enter redireciona para `/busca?busca=...`.
  - Página dedicada `/busca` com `SearchBox` que aceita `initialQuery` e sincroniza query string.
- Admin product form
  - Salvar tudo com um único botão (formulário centralizado).
  - Edição inline de variantes com persistência de inventário em `inventory_item` (usar transação).
- Feedback UX
  - Substituir console.log por toasts (usamos `sonner`) para mensagens de sucesso/erro.

6. Comportamento do `SearchBox`
- Busca por nível de variante (pesquisar `product_variant`) e retornar `productSlug`, `variantSlug`, `imageUrl`, `priceInCents`.
- Implementar debounce, suporte a cancelamento (AbortController) e navegação por teclado (seta cima/baixo, Enter).
...
11. Ferramentas e comandos úteis
...
Se quiser, posso ajustar este `instructions.md` para incluir regras adicionais (ex.: convenções de branch, regras de revisão, template de PR). Quer que eu adicione um template de PR e um modelo de commit também?
