...
export default function ProductForm({ initial = {}, categories = [], onSubmit }: Props) {
  ...
  const addVariant = () =>
    setVariants(s => [...s, { name: '', priceInCents: 0, sizes: [], stock: 0 }]);
  const removeVariant = (idx: number) => setVariants(s => s.filter((_, i) => i !== idx));
  const updateVariant = (idx: number, v: Variant) =>
    setVariants(s => s.map((it, i) => (i === idx ? v : it)));
  ...
  function ProductPreview() {
    const mainVariant = variants[previewVariantIndex] ??
      variants[0] ?? { name: '', priceInCents: 0, imageUrl: '' };
    const price = formatCurrency(mainVariant.priceInCents ?? 0);
    return (
      <div className="rounded-lg border bg-white p-4 shadow-sm">
        {variants && variants.length > 0 && (
          <div className="mb-3">
            <label className="text-muted-foreground mb-1 block text-sm">Visualizar variante</label>
            <select
              className="input w-full"
              value={String(previewVariantIndex)}
        ...
        <div className="relative flex h-56 w-full items-center justify-center overflow-hidden rounded bg-gray-50">
        ...
        <div className="mt-4">
          ...
          <p className="text-muted-foreground text-sm">{mainVariant.name || 'Variante'}</p>
            ...
            <div className="mt-3 flex gap-2 overflow-x-auto">
              {variants.map((v, i) => (
                <button
                  ...
                  className={`h-14 w-14 overflow-hidden rounded border ${i === previewVariantIndex ? 'ring-primary ring-2' : ''}`}
                >
                  {v.imageUrl ? (
  ...
  return (
    <form
      onSubmit={async e => {
        ...
        if (!description || !description.trim())
          nextErrors.push('Descrição do produto é obrigatória');
        ...
        variants.forEach((v, vi) => {
          ...
          if (!v.color || !v.color.trim())
            nextErrors.push(`Variante "${v.name || vi + 1}": cor é obrigatória`);
          if (v.priceInCents == null || Number.isNaN(v.priceInCents) || v.priceInCents < 0)
            nextErrors.push(`Variante "${v.name || vi + 1}": preço inválido`);
          if (!v.imageUrl || !v.imageUrl.trim())
            nextErrors.push(`Variante "${v.name || vi + 1}": imagem (URL) é obrigatória`);
          if (v.sizes && v.sizes.length > 0) {
            v.sizes.forEach((s, si) => {
              if (!s.size || !s.size.trim())
                nextErrors.push(
                  `Variante "${v.name || vi + 1}" tamanho #${si + 1}: tamanho é obrigatório`,
                );
              if (s.quantity == null || Number.isNaN(s.quantity) || s.quantity < 0)
                nextErrors.push(
                  `Variante "${v.name || vi + 1}" tamanho "${s.size}": quantidade inválida`,
                );
            });
          } else {
            if (v.stock == null || Number.isNaN(v.stock) || v.stock < 0)
              nextErrors.push(
                `Variante "${v.name || vi + 1}": informe estoque ou adicione tamanhos`,
              );
          }
        });
    ...
    >
      ...
      <div className="space-y-4 lg:col-span-2">
        <div>
          ...
          <Input
            ...
            placeholder="Ex: Tênis Esportivo Modelo X"
          />
        </div>
        ...
        <div>
          ...
          <div className="text-muted-foreground rounded bg-gray-50 p-2 text-sm">{slug || '—'}</div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={5}
            placeholder="Descreva o produto: material, ajuste, recomendações de uso, etc."
          />
        </div>
          ...
          {categoryId === '__other__' && (
            <div className="mt-2 flex gap-2">
              <Input
                ...
                aria-label="Nome nova categoria"
              />
              ...
              return (
                <div key={key} className="rounded-lg border bg-white p-4 shadow-sm">
                  <div className="flex gap-4">
                    <div className="flex h-24 w-24 items-center justify-center overflow-hidden rounded bg-gray-50">
                      {v.imageUrl ? (
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-muted-foreground text-sm">Cor: {v.color || '-'}</div>
                        </div>
                        <div className="text-sm">
                          Preço: <strong>{((v.priceInCents ?? 0) / 100).toFixed(2)}</strong>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-3">
                        <div>
                          ...
                          <Input
                            ...
                            placeholder="Nome da variante (ex: Branco)"
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            placeholder="Preço em centavos (ex: 1999)"
                          />
                        </div>
                      </div>
                      ...
                      <div className="mt-3">
                        <Input
                          ...
                          placeholder="URL da imagem da variante"
                        />
                      </div>
                    </div>
                  </div>
                              ...
                              <tr key={si} className="border-t">
                                <td className="py-2">
                                  <Input
                                    value={s.size}
                                    onChange={e => {
                                      const next = (v.sizes || []).map((it, i) =>
                                        i === si ? { ...it, size: e.target.value } : it,
                                      );
                            ...
                            <tr className="border-t">
                              ...
                              <td className="py-2">
                                <Input
                                  ...
                                  type="number"
                                  value={String(newSizeInputs[idx]?.quantity ?? '')}
                                  onChange={e =>
                                    setNewSizeInputs(s => ({
                                      ...s,
                                      [idx]: {
      ...
      <div className="lg:sticky lg:top-24 lg:col-span-1">
        ...
        <div className="mt-4">
          <LoadingButton isLoading={submitting} type="submit">
            Salvar
          </LoadingButton>
        </div>
      </div>
