...
import { AddressForm, type AddressFormValues } from "@/app/cart/identification/components/address-form";
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                ...
                {addresses?.map((a: typeof shippingAddressTable.$inferSelect) => (
                  <div key={a.id} className="rounded-md border p-3 flex items-start justify-between">
                    <div className="text-sm">{formatAddress(a)}</div>
                    <div className="ml-4">
                      <Dialog open={deletingId === a.id} onOpenChange={(o) => setDeletingId(o ? a.id : null)}>
                        ...
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <DialogDescription>Essa ação não pode ser desfeita. Deseja remover este endereço da sua conta?</DialogDescription>
                          </DialogHeader>
                          <DialogFooter>
                            <Button variant="outline" onClick={() => setDeletingId(null)}>Cancelar</Button>
                            <Button variant="destructive" onClick={() => handleDelete(a.id)} disabled={deleteMutation.isPending}>
