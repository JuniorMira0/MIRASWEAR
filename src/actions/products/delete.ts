...
export async function deleteProduct(data: z.infer<typeof DeleteProductSchema>) {
  ...
  const variants = await db.query.productVariantTable.findMany({
    where: (t, { eq }) => eq(t.productId, data.id),
  });
  const variantIds = variants.map(v => v.id);

  if (variantIds.length > 0) {
    const existingOrder = await db.query.orderItemTable.findFirst({
      where: (t, { inArray }) => inArray(t.productVariantId, variantIds),
    });
    if (existingOrder) {
      throw new Error(
        'Não é possível remover produto porque há pedidos que referenciam suas variantes.',
      );
    }
  }
  ...
  await db
    ...
    .set({ isActive: false })
    .where(eq(productVariantTable.productId, data.id));
